import { type FC, useRef, useState, useLayoutEffect, useMemo, useEffect } from "react";
                    ...
                    for (let radius = 1; radius <= 10; radius++) {
                        for (let dx = -radius; dx <= radius; dx++) {
                            for (let dy = -radius; dy <= radius; dy++) {
                                ...
                                const isSpotColliding = buildingsMap.buildings.some(b => {
                                    const bMinX = b.bottomLeftX;
                                    const bMaxX = b.bottomLeftX + b.building.baseWidth - 1;
                                    const bMinY = b.bottomLeftY - b.building.baseHeight + 1;
                                    const bMaxY = b.bottomLeftY;
                                ...
                                if (!isSpotColliding) {
                                    foundX = unit.x + (dx * tileSize);
                                    foundY = unit.y + (dy * tileSize);
                                    found = true;
                                    break;
                                }
                            }
                    ...
                    {(() => {
                        ...
                        buildingsMap.buildings.forEach((building, index) => {
                            renderables.push({
                                ...
                                node: <BuildingComponent transparentOnHover={!!placingBuilding} key={`building-${index}`} building={building} tileSize={tileSize} onClick={onBuildingClick} />,
                            });
                        });
